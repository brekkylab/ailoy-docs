...
To see the examples, take a look at the **[out-of-the-box tools](#out-of-the-box-tools)**.
...
If the API does not require authentication or needs custom one, the authentication field may not be contained.
...
**Behavior**: Defined by preset behavior implemented inside Ailoy. The only thing defined outside of Ailoy is the JMESPath query string(`outputPath`) to process the raw results of the function into useful data.
...
**Behavior**: A Python or Node.js function or coroutine that will be called with arguments from the AI.
...
### Frankfurters (Currency exchange rates)
...
with Agent(...) as agent:
  ...
  agent.query("Find me some articles about 'Agentic AI'.")
```
