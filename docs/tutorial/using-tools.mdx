...
When tools are defined as presets, you can easily inject them into the LLM using the add_tools_from_preset function.
...
You can see that the agent uses the Frankfurters API to incorporate real-time exchange rate information into its response.
...
If the tool what you are looking for doesn't exist, or you want more fine-grained control, you can define your own tool from scratch.
Ailoy provides a simple API for creating custom tools.

The following example shows how to manually define the Frankfurter API tool we saw earlier.
...
For the schema of tool definitions, see the [Tools](../concepts/tools) section.
